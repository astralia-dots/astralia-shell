...
- **Reusing `arc_gauge` at a larger diameter must scale the stroke by the reference's stroke/diameter ratio.** `lock`'s ~3x gauges kept `dashboard`'s 6px stroke and rendered as a hairline; `kLockResGaugeStrokeRatio` (`6/68`) fixes it.
...
- **A single-instance overlay bound to one output must not have its open-state read per-monitor.** Every monitor's `bar_paint` saw `logout`/`dashboard` open and expanded everywhere; gate on `bound_output() == mon.output.wl`.
