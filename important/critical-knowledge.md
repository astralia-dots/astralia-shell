...
- **A module's background opacity can diverge from the process-wide `overlay` token.** The launcher menu fill uses `palette::base_alpha80` (`0.8`) instead of the shared `overlay` (`0.92`).
...
- **`visualizer`'s audio chain renders into `GL_R16` targets for the per-write `[0,1]` clamp.** `GL_EXT_texture_norm16` provides R16; probe the extension string and FBO completeness, degrading to a cleared window if absent.
...
- **`ncs`'s blob bypasses `Node`/`Scene` and draws accumulation as additive `GL_POINTS` (`GL_ONE, GL_ONE`) into a `GL_RGBA` texture.** ES 2.0 has no `imageAtomicAdd`; `sphere2_main.glsl` reads it back via `texture2D`, following `ncs`'s `opengles2` branch.
...
- **A CPU-built particle grid should rebuild only when its inputs change, not every frame.** `ensure_particle_grid` caches the `(canvas, particle_thin)` pair it last built against; `particle_thin` is a live slider.
...
- **Use a separate `accent_alt` token for tile/chip selection borders, not `accent`.** `accent` is reserved for other UI like the nav rail and toggle track.
...
- **A pure-logic function needed by a second feature module should move to `service/`.** The scaled-decode helper left `wallpaper.cpp` for `media_service` once the settings tab needed it too.
...
- **A bar widget file must be named for its capsule, not bundled into a sibling.** `cpu_pill`/`tray_pill` lived in `dashboard_widget.cpp`; the five status segments share one capsule, so one `status_widget`.
