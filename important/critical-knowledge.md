...
- **A bar widget file must be named for its capsule, not bundled into a sibling.** `cpu_pill`/`tray_pill` lived in `dashboard_widget.cpp`; the four status segments share one capsule, so one `status_widget`.
