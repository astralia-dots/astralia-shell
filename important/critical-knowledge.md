...
- **The shell runs one OpenGL ES 2.0 context everywhere, shaders are `#version 100`.** EGL config asks `EGL_OPENGL_ES2_BIT`; reverted from a prior ES 3.2 context (two creation sites: `wayland_registry.cpp` and `visualizer.cpp`'s render-thread share context, must change in lockstep).
- **GLSL ES 1.00 requires a compile-time-constant loop bound; a uniform-derived one doesn't compile.** Non-constant bounds (`glow.directions`, `audio_smooth.frag`'s `smin`/`smax`) need a fixed `MAX_*` cap plus an early `break`, sized generously past the config's actual live range.
- **GLSL ES 1.00 has no `matNxM` typenames (`mat3x3`, `mat4x4`), only `mat2`/`mat3`/`mat4`.** `common.glsl`'s `rotateX`/`rotateY`/`rotateZ` used `mat3x3`, valid under the old `#version 320 es`; it silently broke every sphere-pipeline program (`sphere1`/`sphere2`/`glow`) after the ES2 downgrade, since all three concatenate `common.glsl`.
- **GLSL ES 1.00 has no array constructors (`float[8](...)`) or whole-array assignment.** `defaults.glsl`'s `setAudio()` kept the ES 3.2 form; fix is per-element assignment after a bare declaration, like `sphere1_vert_main.glsl`'s own `setAudio()` already does.
...
- **`glTexImage2D`'s internal format must equal its data format on ES 2.0 (`GL_RGBA`, not `GL_RGBA8`).** Sized internal formats are an ES 3 core feature; `GL_RGBA8` compiles (it's just an enum) but is the wrong token for `glTexImage2D` here.
...
- **A single-pass module shader can ride `Renderer::draw_custom` instead of owning FBOs.** `logout`'s `thunder_burst`/`thunder_shock` compile lazily, reuse `kRendererQuadVs` and `quad_vbo_`, and draw only their bounding box.
...
- **On-demand bar panels track live content height through one animated value (`PanelHeightReveal`), not a frozen height.** A `closing` latch keeps a live recompute from re-targeting the value the close collapse animates to zero.
...
- **`visualizer`'s audio chain renders into `GL_R16` targets for the per-write `[0,1]` clamp.** `GL_EXT_texture_norm16` gives R16 on both ES 2.0 and ES 3.2; probe the extension string explicitly (ES 2.0 drivers vary more) plus FBO-completeness, and degrade to a cleared window if absent.
- **ES 2.0's `texelFetch`-free GLSL means exact-texel reads go through `texture2D` with `GL_NEAREST` filtering, not integer coords, but the `+0.5` UV offset only belongs on an explicit integer index, never on `gl_FragCoord`.** `gl_FragCoord.x` for the fragment covering texel `i` is already `i + 0.5`; dividing it straight by the texture width already lands on that texel's center. `audio_smooth.frag`'s `fetch_sample`/`ncs`'s `AUDIO_TEXEL` macro take a raw integer `idx` and correctly do `(idx+0.5)/width`. `audio_pass`/`gravity`/`average` used to add a second `+0.5` on top of `gl_FragCoord.x`, landing on the boundary with texel `i+1` instead (`GL_NEAREST` there is implementation-defined, which is how this passed unnoticed) — `ncs`'s own `audio-pass-1.frag`/`audio-gravity-1.frag`/`audio-average-1.frag` have no such offset; matching them dropped it.
- **`visualizer_shaders::kFullscreenVs` emits only `gl_Position`; a fragment shader paired with it must use `gl_FragCoord`, not a `vUv` varying.** `kBarFs` first declared `in vec2 vUv` and failed to link (no matching output), leaving `bar_ok` false and the visualizer cleared.
- **`visualizer`'s `fps` knob throttles only the sphere; the render thread paces the bars at a fixed `kVisualizerBarFps`.** The knob defaults low on Intel to tame the `ncs` blob; the cheap bar shader inheriting it looked choppy.
- **A temporal average needs an N-FBO ring, not one accumulator.** `visualizer` copies each frame's gravity-decayed spectrum into `ring[i++%5]`, then `average` Hann-weights the five slots.
- **The sphere's point-sprite accumulator (`fbo_tex_[0]`) must be a `GL_FLOAT` render target, not `GL_UNSIGNED_BYTE`.** The shell-evacuation math piles hundreds of overlapping splats onto one pixel (the inner ~55% of the particle disc converges onto a thin ring); an 8-bit `UNORM` target clamps at `1.0`, starving `sphere2_main.glsl`'s `actualDepth`-driven brightness curve and killing the glow pass that reads its output. `GL_OES_texture_float`/`GL_EXT_color_buffer_float`/`GL_EXT_float_blend` (additive blending into it) cover this on ES 2.0; probe before relying on it elsewhere, this hardware has all three.
- **`ncs`'s blob bypasses `Node`/`Scene` entirely, and no longer uses image load/store.** ES 2.0 has no `imageAtomicAdd`/`imageAtomicExchange` (that was ES 3.1+, not just 3.0); the accumulation pass is now a real `GL_POINTS` draw (`sphere1_vert_head.glsl`/`sphere1_vert_main.glsl`, one vertex per surviving `particleThin`-dropout grid pixel, built host-side in `SphereVisualizer::ensure_particle_grid`) additive-blended (`glBlendFunc(GL_ONE, GL_ONE)`) into a plain `GL_RGBA` texture that `sphere2_main.glsl` reads back via `texture2D`. Reference port: `~/workspace/ncs`'s `opengles2` branch (`ncs-1.vert`/`ncs-1.frag`/`ncs-2.frag`), which solved the identical problem first — `ncs` is the upstream this blob was ported from. `colorTracking` was already `0` (dead code) in the ES 3.2 version, so the 4 unused color-channel atomic images weren't ported either, only the depth accumulator.
- **A point-sprite's footprint size must match the original loop's coordinate space, not the loop's raw bound.** The deleted `sphere1_main.glsl`'s `-size..size` loop added its `(i, j)` offset *before* dividing by `baseForm.scale`, so `gl_PointSize` needs `2*particle.size/baseForm.scale.x`, not `2*particle.size`.
- **A per-frame CPU-rebuilt particle grid only needs rebuilding when its inputs change, not every frame.** `ensure_particle_grid` caches the `(canvas, particle_thin)` pair it last built against; `particle_thin` is a live settings slider, so canvas-resize isn't the only trigger.
...
- **The visualizer presents its canvas as a scoped textured quad, not a full `glBlitFramebuffer`.** An earlier whole-surface quad present stuttered; scoping it to just the ~35%-of-window canvas rect avoided the fill cost.
- **`ncs`'s grid-step divisor is `0` when `baseForm.numParticles.z == 1` (the `visualizer` default); only Intel turns that into a GPU hang.** `int(Inf)` clamps high on Mesa/NVIDIA (loop exits), but is `0` then `-1` on Intel, running the loop backwards forever.
- **`kNcs1Main` must carry `ncs`'s grid-step guard clauses verbatim, not just the divisor fix.** `max(denom, vec3(0.0001))`, `max(spaces.x/y, 1)`, and the `numParticles.z <= 1.0` ternary branch for `spaces.z`.
- **`ncs` never `glClear`s; `visualizer`'s `0.7` black backdrop is painted host-side instead, not in the ported shader.** The render thread `glClear`s the framebuffer to `window_backdrop` with alpha `* fade`, then blends the premultiplied glow quad over it.
- **Backdrop compositing used to live in the `glow` shader itself, via `u_backdrop`, but that path is gone.** It broke on Mesa when `pow(0,0)` made a pixel `NaN`; the host-side clear is immune to that.
...
- **On Intel, the untuned `ncs` blob at ~110ms/frame meant the `xdg_toplevel` never committed a buffer at all.** Per-device knobs exist because the blob is heavy; the render loop just runs at whatever rate the hardware gives.
...
- **`fps` is a `VisualizerParams` knob consumed by both the render thread's frame step and the gravity-decay divisor.** Both read the live value, so lowering `fps` stays wall-clock-consistent, matching `ncs`'s own `nanosleep`-paced loop.
...
- **`visualizer`'s render thread self-paces and owns every `wl_surface` request; the poll thread never drives its frames.** It loops on `cv.wait_until` at `1/params.fps`, doing its own capture/FFT/render/`eglSwapBuffers` each iteration.
...
- **The six ported wallpaper transition shaders take a CPU-computed uv scale+offset (`vec4`), not an in-shader fill-mode block.** astralia-shell has two fill modes (Crop/Fit); `transition_uv` reproduces the centred-cover/contain placement, returning `u_fill` for out-of-`[0,1]` samples.
...
- **`MarqueeTextState::marqueeing` means "currently scrolling," not "text overflows the box."** `draw_marquee_text` must clip on `tex->width > w` directly; gating the clip on `marqueeing` skips it whenever scrolling is suppressed, letting long text overflow.
...
- **A per-visual special-cased render path causes divergence from an otherwise uniform design.** astralia-shell's earlier per-visual special-casing caused divergence; one shared Renderer/Scene thread, every style an ordinary `Node` on one opacity pipeline, resolved it.
...
- **`dashboard`'s circular arc gauge moved to `render/arc_gauge` whole, geometry and colors as caller params, once `lock` needed it.** Callers pass their own diameter/stroke/colors; only the arc is cached inside, not caller textures.
...
- **The same flat track+fill progress bar was reimplemented three times, with two byte-identical `kWarnColor` copies.** Moved to `render/progress_bar` with `min_fill_w` a caller param, since `osd`'s OSD has no width floor and panels do.
...
- **Use a separate `accentAlt` token for tile/chip selection borders, not `accent`.** `accent` is reserved for other UI like the nav rail and toggle track.
