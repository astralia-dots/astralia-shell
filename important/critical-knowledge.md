...
- **Default EGL swap interval (`1`) makes `eglSwapBuffers` block the poll thread indefinitely on NVIDIA when the compositor withholds the buffer.** e.g. occluded by a fullscreen overlay or under `ext-session-lock`; `main()` sets `eglSwapInterval(egl_display, 0)` once, after the first `eglMakeCurrent`.
