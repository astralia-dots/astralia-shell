...
- **`visualizer`'s audio chain renders into `GL_R16` targets for the per-write `[0,1]` clamp.** `GL_EXT_texture_norm16` gives R16; probe it for colour-renderability at init and degrade to a cleared window if absent.
