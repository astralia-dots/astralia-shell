...
- **`wl_surface.frame` callbacks already pace every surface, so `eglSwapInterval(0)` doesn't cause tearing or a runaway loop.** Every `eglMakeCurrent`/`eglSwapBuffers` also checks its return and skips the frame on `EGL_FALSE`, the noctalia pattern.
...
- **`visualizer`'s ported GLava audio chain renders into `GL_R16` targets for the per-write `[0,1]` clamp.** `GL_EXT_texture_norm16` gives R16; probe it for colour-renderability at init and degrade to a cleared window if absent.
...
- **A GLava-style temporal average needs an N-FBO ring, not one accumulator.** `visualizer` copies each frame's gravity-decayed spectrum into `ring[i++%5]`, then `average` Hann-weights the five slots.
...
- **The `accent`-hued blob/glow is a deliberate kokusei override, fed through the `u_accent` uniform, not a shader-literal edit.** This keeps every `ncs` shader-literal constant verbatim; the `0.7` black backdrop is painted host-side instead, not in-shader.
...
- **Confirmed on Hyprland `v0.56.2`: this is an upstream wlroots/Hyprland bug, not a kokusei bug; no client-side fix exists.** The compositor never sends the event to route. See `hyprwm/Hyprland` discussion #13116 and `swaywm/wlroots` issue #233.
...
- **kokusei has no runtime shader preprocessor; flatten ported multi-file shaders at authoring time.** `visualizer` inlines every `#include` and hand-expands `#expand` into `assets/shaders/visualizer/sphere/*.glsl` fragment files, concatenated at runtime by `visualizer_shaders.cpp`.
