...
- **ES 2.0's `texelFetch`-free GLSL means exact-texel reads go through `texture2D` with a `(coord+0.5)/width` UV and `GL_NEAREST` filtering, not integer coords.** The audio chain (`audio_pass`/`gravity`/`average`/`smooth`) and the sphere's audio-driven noise all read this way now; the textures were already `GL_NEAREST`-filtered, so this is bit-exact, not an approximation.
