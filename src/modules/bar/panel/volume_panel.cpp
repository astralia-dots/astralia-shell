...
std::vector<PanelRow> build_rows(const PipewireState &pw) {
    ...
    for (const PwNodeEntry *sink : pipewire_sinks(pw))
        rows.push_back({RowKind::OutputDeviceRow, kVolumeDeviceRowHeight, sink});

    rows.push_back({RowKind::InputDeviceHeader, kVolumeSectionHeaderHeight});
    for (const PwNodeEntry *source : pipewire_sources(pw))
        rows.push_back({RowKind::InputDeviceRow, kVolumeDeviceRowHeight, source});
...
bool volume_panel_init_egl(VolumePanelState &state, Renderer &renderer, PipewireState &pw, EGLDisplay display, EGLConfig config, EGLContext context) {
    ...
    state.base.frame_clock.draw = [&state, &pw] {
        volume_panel_paint(state, pw, state.pending_pill_center_x, state.pending_bar_height, state.pending_bar_top_margin);
    };
    return true;
}
...
void volume_panel_handle_scroll(VolumePanelState &state, const PipewireState &pw, double dy) {
    state.scroll_offset =
        panel_clamp_scroll(state.scroll_offset, static_cast<float>(dy), volume_panel_detail::content_height(volume_panel_detail::build_rows(pw)), state.visible_content_height);
}
...
void volume_panel_paint(VolumePanelState &state, PipewireState &pw, float pill_center_x, float bar_height, float bar_top_margin) {
    ...
    panel_draw_header(root, state.tcache, scale, "Volume", panel_x, panel_y, panel_w, state.click_regions);
    ...
    const std::string sink_desc = sink_it != pw.nodes.end() ? (sink_it->second.description.empty() ? sink_it->second.name : sink_it->second.description) : "";
    const std::string source_desc = source_it != pw.nodes.end()
                                        ? (source_it->second.description.empty() ? source_it->second.name : source_it->second.description)
                                        : "";
    ...
    for (size_t i = 0; i < rows.size(); ++i) {
        ...
        auto draw_label_row = [&](const char *title, const std::string &desc) {
            ...
            float title_w = 0.0f;
            if (title_tex) {
                node_add_texture(clip, rx(content_x), ry(y + (row_h - title_tex->height) / 2.0f), *title_tex, white);
            ...
            if (!desc.empty()) {
                ...
                if (desc_tex)
                    node_add_texture(clip, rx(content_x + title_w), ry(y + (row_h - desc_tex->height) / 2.0f), *desc_tex, dim);
            }
        };

        auto draw_slider_row = [&](float level, bool muted, const std::string &tag) {
            float mute_x = content_x + content_w - kPanelActionButtonSize;
            Rect mute_rect = {mute_x, y + (row_h - kPanelActionButtonSize) / 2.0f, kPanelActionButtonSize, kPanelActionButtonSize};
            node_add_rrect(clip, rx(mute_rect.x), ry(mute_rect.y), mute_rect.w, mute_rect.h, kPanelActionButtonSize / 2.0f, 0.0f, rgba(palette::overlay), kPanelNoBorder);
            const Texture *mute_icon = cached_icon(state.tcache, volume_threshold_icon(muted, level), scale);
            if (mute_icon)
                node_add_texture(clip, rx(mute_rect.x + (mute_rect.w - mute_icon->width) / 2.0f), ry(mute_rect.y + (mute_rect.h - mute_icon->height) / 2.0f), *mute_icon, white);
            ...
            if (pct_tex)
                node_add_texture(clip, rx(pct_x + kVolumePercentLabelWidth - pct_tex->width), ry(y + (row_h - pct_tex->height) / 2.0f), *pct_tex, dim);

            float slider_right = pct_x - kVolumeSliderRightGap;
            Rect slider_rect = {content_x, y, slider_right - content_x, row_h};
            Rect slider_local = {rx(slider_rect.x), ry(slider_rect.y), slider_rect.w, slider_rect.h};
            draw_slider_track(clip, state.click_regions, slider_local, slider_rect, kVolumeSliderTrackHeight, muted ? 0.0f : level, muted, tag);
        };
        ...
        case RowKind::Divider:
            node_add_rect(clip, rx(content_x), ry(y), content_w, 1.0f, rgba(palette::text_alpha06));
            break;
        ...
        case RowKind::InputDeviceHeader: {
            ...
            if (t)
                node_add_texture(clip, rx(content_x), ry(y + row_h - t->height), *t, white);
            break;
        }
        case RowKind::AppRow: {
            ...
            float mute_x = content_x + content_w - kPanelActionButtonSize;
            Rect mute_rect = {mute_x, y, kPanelActionButtonSize, kPanelActionButtonSize};
            node_add_rrect(clip, rx(mute_rect.x), ry(mute_rect.y), mute_rect.w, mute_rect.h, kPanelActionButtonSize / 2.0f, 0.0f, rgba(palette::overlay), kPanelNoBorder);
            const Texture *mute_icon = cached_icon(state.tcache, volume_threshold_icon(entry.muted, entry.level), scale);
            if (mute_icon)
                node_add_texture(clip, rx(mute_rect.x + (mute_rect.w - mute_icon->width) / 2.0f), ry(mute_rect.y + (mute_rect.h - mute_icon->height) / 2.0f), *mute_icon, white);
            ...
            Rect slider_local = {rx(slider_rect.x), ry(slider_rect.y), slider_rect.w, slider_rect.h};
            draw_slider_track(clip, state.click_regions, slider_local, slider_rect, kVolumeAppSliderTrackHeight, entry.muted ? 0.0f : entry.level, entry.muted, tag);
            break;
        }
        case RowKind::OutputDeviceRow:
        case RowKind::InputDeviceRow: {
            const PwNodeEntry &entry = *row.entry;
            bool is_default = entry.id == (row.kind == RowKind::OutputDeviceRow ? pw.default_sink_id : pw.default_source_id);
            std::string label =
            ...
            node_add_rrect(clip, rx(content_x), ry(dot_y), kVolumeDeviceIndicatorSize, kVolumeDeviceIndicatorSize, kVolumeDeviceIndicatorRadius, 0.0f, is_default ? rgba(palette::accent) : rgba(palette::text_alpha20), kPanelNoBorder);
            if (is_default) {
                float inner_off = (kVolumeDeviceIndicatorSize - kVolumeDeviceIndicatorDotSize) / 2.0f;
                node_add_rrect(clip, rx(content_x + inner_off), ry(dot_y + inner_off), kVolumeDeviceIndicatorDotSize, kVolumeDeviceIndicatorDotSize, kVolumeDeviceIndicatorDotRadius, 0.0f, white, kPanelNoBorder);
            }

            float text_x =
                content_x + kVolumeDeviceIndicatorSize + kPanelRowIconGap;
            int text_w_px = static_cast<int>(std::max(0.0f, content_x + content_w - text_x));
            const Texture *label_tex =
                cached_text_clipped(state.tcache, label, scale, text_w_px);
            if (label_tex)
                node_add_texture(clip, rx(text_x), ry(y + (row_h - label_tex->height) / 2.0f), *label_tex, is_default ? rgba(palette::accent) : white);

            Rect row_rect = {content_x, y, content_w, row_h};
            state.click_regions.push_back({PanelClickKind::DeviceSelect, row_rect, std::to_string(entry.id)});
            break;
        }
        case RowKind::Spacer:
            break;
