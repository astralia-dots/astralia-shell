...
bool control_center_panel_create_surface(ControlCenterPanelState &state, wl_compositor *compositor, zwlr_layer_shell_v1 *layer_shell, wl_output *output) {
    return overlay_panel_create_surface(state.base, compositor, layer_shell, "astralia-shell-control-center", output);
}
...
bool control_center_panel_init_egl(ControlCenterPanelState &state, Renderer &renderer, WaylandState &app, EGLDisplay display, EGLConfig config, EGLContext context) {
    ...
    state.base.frame_clock.draw = [&state, &app] {
        control_center_panel_paint(state, app, state.pending_bar_height, state.pending_bar_top_margin);
    };
...
void apply_brightness_drag(ControlCenterPanelState &state, WaylandState &app, double px) {
...
void control_center_panel_handle_click(ControlCenterPanelState &state, WaylandState &app, double px, double py) {
    ...
    for (const PanelClickRegion &region : state.click_regions) {
        ...
        case PanelClickKind::Close:
            control_center_panel_toggle(state);
            break;
...
float draw_battery_card(Node *root, TextureCache &tcache, int32_t scale, float x, float y, float w, const UpowerState &upower) {
    ...
    PanelCardChrome chrome =
        panel_draw_card(root, tcache, scale, x, y, w, content_h, "Battery");
...
float draw_media_card(Node *root, TextureCache &tcache, int32_t scale, float x, float y, float w, const MprisState &mpris, std::unordered_map<std::string, Texture> &art_cache, AnimationManager &anim, MarqueeTextState &title_marquee, MarqueeTextState &artist_marquee, std::vector<PanelClickRegion> &regions) {
    ...
    PanelCardChrome chrome =
        panel_draw_card(root, tcache, scale, x, y, w, content_h, "Media");
    float cx = chrome.content_x, cy = chrome.content_y;
    float content_w = w - 2 * kCardHorizontalPadding;
...
float draw_volume_card(Node *root, TextureCache &tcache, int32_t scale, float x, float y, float w, const PipewireState &pw, std::vector<PanelClickRegion> &regions) {
    ...
    PanelCardChrome chrome =
        panel_draw_card(root, tcache, scale, x, y, w, content_h, "Volume");
    float cx = chrome.content_x, cy = chrome.content_y;
    float content_w = w - 2 * kCardHorizontalPadding;
...
float draw_brightness_card(Node *root, TextureCache &tcache, int32_t scale, float x, float y, float w, bool present, float level, std::vector<PanelClickRegion> &regions) {
    ...
    PanelCardChrome chrome =
        panel_draw_card(root, tcache, scale, x, y, w, content_h, "Brightness");
    float cx = chrome.content_x, cy = chrome.content_y;
    float content_w = w - 2 * kCardHorizontalPadding;
...
void control_center_panel_paint(ControlCenterPanelState &state, WaylandState &app, float bar_height, float bar_top_margin) {
    ...
    float battery_y = content_y + kControlCenterCardGap;
    float battery_h =
    ...
    float brightness_y = content_y + kControlCenterCardGap;
    ...
    float volume_y = content_y + kControlCenterCardGap;
    ...
    float media_y = content_y + kControlCenterCardGap;
