...
bool bluetooth_panel_init_egl(BluetoothPanelState &state, Renderer &renderer, BluetoothState &bt, EGLDisplay display, EGLConfig config, EGLContext context) {
    ...
    state.base.frame_clock.draw = [&state, &bt] {
        bluetooth_panel_paint(state, bt, state.pending_pill_center_x, state.pending_bar_height, state.pending_bar_top_margin);
    };
    return true;
}
...
void bluetooth_panel_toggle(BluetoothPanelState &state, BluetoothState &bt, float pill_center_x) {
...
void bluetooth_panel_handle_scroll(BluetoothPanelState &state, BluetoothState &bt, double dy) {
    state.scroll_offset =
        panel_clamp_scroll(state.scroll_offset, static_cast<float>(dy), bluetooth_panel_detail::content_height(bluetooth_panel_detail::build_rows(bt)), state.visible_content_height);
}
...
void bluetooth_panel_paint(BluetoothPanelState &state, BluetoothState &bt, float pill_center_x, float bar_height, float bar_top_margin) {
    ...
    float hx =
        panel_draw_header(root, state.tcache, scale, "Bluetooth", panel_x, panel_y, panel_w, state.click_regions);

    if (bt.adapter_present) {
        panel_draw_toggle_switch(root, state.click_regions, hx - 36.0f, header_y + (kPanelHeaderHeight - 20.0f) / 2.0f, 36.0f, 20.0f, 16.0f, 2.0f, bt.powered, PanelClickKind::HeaderToggle, "");
    }
    ...
    for (size_t i = 0; i < rows.size(); ++i) {
        ...
        case RowKind::NoAdapter:
            panel_draw_centered_text(clip, state.tcache, scale, "No adapter found", rx(content_x), ry(y), content_w, row_h, dim);
            break;
        case RowKind::Off:
            panel_draw_centered_text(clip, state.tcache, scale, "Bluetooth is off", rx(content_x), ry(y), content_w, row_h, dim);
            break;
        case RowKind::Empty:
            panel_draw_centered_text(clip, state.tcache, scale, "No paired devices", rx(content_x), ry(y), content_w, row_h, dim);
            break;
        ...
        case RowKind::Device: {
            ...
            if (dev_icon)
                node_add_texture(clip, rx(content_x + kPanelRowIconGap), ry(y + (row_h - dev_icon->height) / 2.0f), *dev_icon, is_connected ? white : fg);
            ...
            Node *tclip = node_add_group(clip, rx(text_left), ry(y), text_right - text_left, row_h, true);
            ...
            const Texture *sub_tex =
                subtitle.empty() ? nullptr : cached_text_clipped(state.tcache, subtitle, scale, text_w_px);

            panel_draw_row_text(tclip, name_tex, sub_tex, row_h, is_connected ? rgba(palette::accent) : white, dim);

            panel_draw_row_actions(clip, state.tcache, scale, state.click_regions, actions, content_x, content_w, y, row_h, panel_x, content_top, is_connected, is_busy, show_forget, info.path);
            break;
        }
        case RowKind::Spacer:
            break;
    ...
    if (!state.sub_mode.empty()) {
        ...
        const char *confirm_label =
            state.sub_mode == "disconnect" ? "Disconnect" : "Forget";
        panel_draw_confirm_subpanel(root, state.tcache, scale, top_label, prompt, confirm_label, panel_x, sub_y, panel_w, state.click_regions);
    }
