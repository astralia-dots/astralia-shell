...
std::vector<const NetworkInfo *> sorted_by_signal(const std::vector<const NetworkInfo *> &in) {
    std::vector<const NetworkInfo *> out = in;
    std::sort(out.begin(), out.end(), [](const NetworkInfo *a, const NetworkInfo *b) {
...
std::vector<PanelRow> build_rows(const NetworkState &net) {
    ...
    if (net.wifi_available && net.wifi_enabled && network_visible_count(net) == 0)
...
float sub_panel_height(const std::string &mode) {
    ...
    float inner = kPanelDialogSpacerHeight + middle + kPanelRowGap + kPanelSubPanelRowHeight + kPanelRowGap;
...
bool network_panel_init_egl(NetworkPanelState &state, Renderer &renderer, NetworkState &net, EGLDisplay display, EGLConfig config, EGLContext context) {
    ...
    state.base.frame_clock.draw = [&state, &net] {
        network_panel_paint(state, net, state.pending_pill_center_x, state.pending_bar_height, state.pending_bar_top_margin);
    };
    return true;
}
...
void network_panel_toggle(NetworkPanelState &state, float pill_center_x) {
    ...
    text_field_type_anim_clear(state.password_anim, state.base.animations, kNetworkPasswordTypeAnimOwnerBase);
...
void network_panel_handle_scroll(NetworkPanelState &state, NetworkState &net, double dy) {
    state.scroll_offset =
        panel_clamp_scroll(state.scroll_offset, static_cast<float>(dy), network_panel_detail::content_height(network_panel_detail::build_rows(net)), state.visible_content_height);
}

void network_panel_open_sub(NetworkPanelState &state, const std::string &mode, const std::string &ssid) {
    if (state.sub_mode == "password" && mode != "password" && state.sync_text_input_focus)
        state.sync_text_input_focus(false);
    state.sub_mode = mode;
    state.sub_ssid = ssid;
    state.password_field.text.clear();
...
void network_panel_handle_key_event(NetworkPanelState &state, NetworkState &net, const KeyEvent &event) {
    if (state.sub_mode == "password") {
        ...
        case TextFieldResult::Changed:
            text_field_type_anim_sync(state.password_anim, state.base.animations, kNetworkPasswordTypeAnimOwnerBase, state.password_field.text);
            return;
...
void network_panel_handle_click(NetworkPanelState &state, NetworkState &net, double px, double py) {
    ...
    for (const PanelClickRegion &region : state.click_regions) {
        ...
        case PanelClickKind::RowConnect: {
            ...
            } else if (info.existing || info.security.empty() || info.security == "--") {
...
void network_panel_paint(NetworkPanelState &state, NetworkState &net, float pill_center_x, float bar_height, float bar_top_margin) {
    ...
    float hx = panel_draw_header(root, state.tcache, scale, "Network", panel_x, panel_y, panel_w, state.click_regions);

    if (net.wifi_available) {
        Rect toggle_rect = panel_draw_toggle_switch(root, state.click_regions, hx - 36.0f, header_y + (kPanelHeaderHeight - 20.0f) / 2.0f, 36.0f, 20.0f, 16.0f, 2.0f, net.wifi_enabled, PanelClickKind::HeaderToggle, "");
        ...
        node_add_rrect(root, scan_rect.x, scan_rect.y, scan_rect.w, scan_rect.h, kPanelActionButtonSize / 2.0f, 0.0f, rgba(palette::overlay), rgba(palette::overlay));
        const Texture *scan_tex =
            cached_icon(state.tcache, icon::refresh, scale);
        if (scan_tex)
            node_add_texture(root, scan_rect.x + (scan_rect.w - scan_tex->width) / 2.0f, scan_rect.y + (scan_rect.h - scan_tex->height) / 2.0f, *scan_tex, net.scanning ? rgba(palette::accent) : white);
        state.click_regions.push_back({PanelClickKind::HeaderAction, scan_rect, ""});
    }
    ...
    auto draw_row = [&](Node *clip, const PanelRow &row, float y, float rx_origin, float ry_origin) {
        auto rx = [&](float v) { return v - rx_origin; };
        auto ry = [&](float v) { return v - ry_origin; };
        float row_h = row.height;
        switch (row.kind) {
        case RowKind::Error: {
            node_add_rrect(clip, rx(content_x), ry(y), content_w, row_h, 8.0f, 0.0f, rgba(palette::critical_alpha15), kPanelNoBorder);
            const Texture *warn =
                cached_icon(state.tcache, icon::alert_triangle, scale);
            float tx = content_x + kPanelRowIconGap;
            if (warn) {
                node_add_texture(clip, rx(tx), ry(y + (row_h - warn->height) / 2.0f), *warn, rgba(palette::critical));
            ...
            if (err_tex)
                node_add_texture(clip, rx(tx), ry(y + (row_h - err_tex->height) / 2.0f), *err_tex, rgba(palette::critical));
            Rect err_close = {content_x + content_w - kPanelRowIconGap - close_w, y + (row_h - close_w) / 2.0f, close_w, close_w};
            const Texture *err_close_tex =
                cached_icon(state.tcache, icon::close, scale);
            if (err_close_tex)
                node_add_texture(clip, rx(err_close.x + (close_w - err_close_tex->width) / 2.0f), ry(err_close.y + (close_w - err_close_tex->height) / 2.0f), *err_close_tex, rgba(palette::critical));
            state.click_regions.push_back({PanelClickKind::ErrorClose, err_close, ""});
            break;
        }
        case RowKind::Ethernet: {
            node_add_rrect(clip, rx(content_x), ry(y), content_w, row_h, 8.0f, 0.0f, rgba(palette::accent_alpha19), kPanelNoBorder);
            const Texture *t = cached_text(state.tcache, "Ethernet", scale);
            if (t)
                node_add_texture(clip, rx(content_x + (content_w - t->width) / 2.0f), ry(y + (row_h - t->height) / 2.0f), *t, white);
            break;
        }
        case RowKind::NoAdapter:
        case RowKind::Disabled:
            panel_draw_centered_text(clip, state.tcache, scale, row.kind == RowKind::NoAdapter ? "No Wi-Fi adapter found" : "WiFi is disabled", rx(content_x), ry(y), content_w, row_h, dim);
            break;
        case RowKind::Scanning: {
            std::string dots(static_cast<size_t>(net.scan_dot_step), '.');
            panel_draw_centered_text(clip, state.tcache, scale, "Scanning" + dots, rx(content_x), ry(y), content_w, row_h, dim);
            break;
        }
        ...
        case RowKind::Device: {
            ...
            if (sig)
                node_add_texture(clip, rx(content_x + kPanelRowIconGap), ry(y + (row_rect_h - sig->height) / 2.0f), *sig, fg);

            bool show_forget = info.existing && !is_connected && !is_busy;
            PanelRowActionLayout actions = panel_measure_row_actions(state.tcache, scale, is_connected, is_busy, show_forget);
            ...
            float ssid_y = sub_tex ? row_rect_h / 2.0f - ssid_h - 1 : row_rect_h / 2.0f - ssid_h / 2.0f;
            draw_marquee_text(tclip, state.tcache, state.base.animations, state.ssid_marquees[info.ssid], scale, info.ssid, 0.0f, ssid_y, static_cast<float>(text_w_px), ssid_fg);
            if (sub_tex)
                node_add_texture(tclip, 0, row_rect_h / 2.0f + 1, *sub_tex, is_captive ? kNetworkCaptiveFg : dim);

            panel_draw_row_actions(clip, state.tcache, scale, state.click_regions, actions, content_x, content_w, y, row_rect_h, rx_origin, ry_origin, is_connected, is_busy, show_forget, info.ssid);
            break;
        }
        case RowKind::Spacer:
            break;
    ...
    Node *scroll_clip = node_add_group(root, panel_x, scroll_top, panel_w, scroll_visible_h, true);
    {
    ...
    if (!state.sub_mode.empty()) {
        ...
        if (state.sub_mode == "password") {
            float iy = panel_draw_subpanel_top(root, state.tcache, scale, state.sub_ssid, panel_x, sub_y, panel_w, sub_h, state.click_regions);
            ...
            if (state.password_field.text.empty()) {
                ...
                if (ph)
                    node_add_texture(root, field_rect.x + (field_rect.w - ph->width) / 2.0f, field_rect.y + (field_rect.h - ph->height) / 2.0f, *ph, dim);
                caret_x = text_field_row_slide(state.password_row_slide, state.base.animations, kNetworkPasswordRowSlideOwner, field_rect.x + field_rect.w / 2.0f);
                ...
                float dx = text_field_row_slide(state.password_row_slide, state.base.animations, kNetworkPasswordRowSlideOwner, field_rect.x + (field_rect.w - dots_w) / 2.0f);
                for (size_t i = 0; i < n; ++i) {
                    ...
                    node_add_texture_rect(root, dx + (dot - w) / 2.0f + slide, dy - h / 2.0f, w, h, state.password_dot_tex, white);
                    dx += dot + gap;
                }
                caret_x = dx - gap;
            }
            draw_text_field_preedit(root, state.tcache, scale, state.password_field.preedit, field_rect.x + field_rect.w / 2.0f, field_rect.y + field_rect.h + 10.0f, dim);
            Rect pw_caret = {caret_x, dy - 7.0f, 1.5f, 14.0f};
            state.password_field.cursor_rect = pw_caret;
            draw_text_field_caret(root, state.password_field, pw_caret, white, true);
            ...
            float btn_w = (connect_tex ? connect_tex->width : 0) + kPanelDialogButtonPaddingH;
            Rect confirm_rect = {inner_x + (inner_w - btn_w) / 2.0f, iy, btn_w, kPanelConfirmButtonSize};
            float accent_alpha = ok ? 1.0f : kNetworkConnectDisabledAlpha;
            float fill[4] = {palette::accent.r, palette::accent.g, palette::accent.b, accent_alpha};
            node_add_rrect(root, confirm_rect.x, confirm_rect.y, confirm_rect.w, confirm_rect.h, 6.0f, 0.0f, fill, kPanelNoBorder);
            if (connect_tex)
                node_add_texture(root, confirm_rect.x + (confirm_rect.w - connect_tex->width) / 2.0f, confirm_rect.y + (confirm_rect.h - connect_tex->height) / 2.0f, *connect_tex, white);
            if (ok)
                state.click_regions.push_back({PanelClickKind::SubConfirm, confirm_rect, ""});
        } else {
            const char *prompt =
                state.sub_mode == "disconnect" ? "Disconnect?" : "Forget?";
            const char *confirm_label =
                state.sub_mode == "disconnect" ? "Disconnect" : "Forget";
            panel_draw_confirm_subpanel(root, state.tcache, scale, state.sub_ssid, prompt, confirm_label, panel_x, sub_y, panel_w, state.click_regions);
        }
    }
...
void network_panel_text_input_apply_edit(NetworkPanelState &state, const TextInputEdit &edit) {
    ...
    state.password_field.cursor_idle_visible = true;
