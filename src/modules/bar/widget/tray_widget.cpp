...
Pill tray_pill(MonitorOutput &mon) {
    ...
    return Pill{PillId::Tray, &bs.tray_texture, "Tray", nullptr, [&mon, &bs] {
                    ...
                    if (!bs.tray_panel.base.open) {
                        ...
                        bar_paint(mon);
                        overlay_panel_ensure(bs.tray_panel.base, [&] { return tray_panel_create_surface(bs.tray_panel, mon.app->compositor, mon.app->layer_shell, mon.output.wl); }, [&] { return tray_panel_init_egl(bs.tray_panel, mon.app->renderer, mon.app->tray, mon.app->egl_display, mon.app->egl_config, mon.app->egl_context); });
