...
void clock_pill_clicked(MonitorOutput &mon) {
    ...
    if (!bs.clock_panel.base.open) {
        overlay_panel_ensure(bs.clock_panel.base, mon.app->display, [&] {
                return clock_panel_create_surface(bs.clock_panel, mon.app->compositor, mon.app->layer_shell, mon.output.wl);
            }, [&] {
                return clock_panel_init_egl(bs.clock_panel, mon.app->renderer, mon.app->egl_display, mon.app->egl_config, mon.app->egl_context);
            });
