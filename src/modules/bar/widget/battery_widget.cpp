    ...
    return Pill{PillId::Battery, &bs.battery_icon_texture, battery_label(u),
                battery_border_color(u), [&mon, &bs] {
                    ...
                    if (!bs.battery_panel.base.open) {
                        ...
                        bar_paint(mon);
                        overlay_panel_ensure(bs.battery_panel.base, mon.app->display, [&] {
                                return battery_panel_create_surface(bs.battery_panel, mon.app->compositor, mon.app->layer_shell, mon.output.wl);
                            }, [&] {
                                return battery_panel_init_egl(bs.battery_panel, mon.app->renderer, mon.app->upower, mon.app->egl_display, mon.app->egl_config, mon.app->egl_context);
                            });
