...
float workspace_pill_width(WorkspaceWidgetState &wstate, AnimationManager &animations, int ws_id, bool active) {
    ...
    if (it == wstate.was_active.end()) {
        ...
        it->second = active;
        animations.animate(wstate.width_t[ws_id], active ? 1.0f : 0.0f, kWorkspacePillAnimMs, Easing::Linear, [&wstate, ws_id](float v) { wstate.width_t[ws_id] = v; }, {}, kWorkspacePillOwnerBase + static_cast<uint64_t>(ws_id));
    }
    float t = wstate.width_t[ws_id];
    return kWorkspacePillHeight + kWorkspacePillHeight * (kWorkspaceActiveWidthScale - 1.0f) * t;
}

float workspace_row_width(WorkspaceWidgetState &wstate, AnimationManager &animations, const std::vector<Workspace> &ws_list, int active_id) {
    float w = 0;
    bool first = true;
