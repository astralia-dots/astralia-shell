...
PangoFontDescription *font_summary() {
    static PangoFontDescription *d = pango_font_description_from_string("ComicShannsMono Nerd Font Semi-Bold 17");
    return d;
}
...
const Texture &close_glyph_texture() {
    static Texture tex = make_texture_from_raster(rasterize_icon(icon::close, kContentScale, kNotificationCloseIconPx));
    return tex;
}

float notification_entry_height(const NotificationEntry &entry) {
    float content_h =
        std::max(kNotificationUrgencyDotSize, notification_detail_texture_height(entry.app_name_texture));
    if (entry.summary_texture.id)
        content_h += kNotificationContentSpacing + notification_detail_texture_height(entry.summary_texture);
    if (entry.body_texture.id)
        content_h += kNotificationContentSpacing + notification_detail_texture_height(entry.body_texture);
...
void notification_build_content(NotificationEntry &entry) {
    ...
    int content_width = kNotificationSurfaceWidth -
                        ...
                        static_cast<int>(kNotificationCloseHitAreaSize + kNotificationCardContentTrailingGap);
    int32_t scale = kContentScale;
    RasterizedText app_name_text = rasterize_text_with(entry.app_name.empty() ? "Notification" : entry.app_name, font_app_name(), scale, content_width);
    entry.app_name_texture = make_texture_from_raster(app_name_text);

    if (!entry.summary.empty()) {
        RasterizedText summary_text = rasterize_text_with(entry.summary, font_summary(), scale, content_width);
...
void notification_start_exit(NotificationRenderModel &service, uint32_t id) {
    auto it = std::find_if(service.entries.begin(), service.entries.end(), [id](const NotificationEntry &e) { return e.id == id; });
...
void notification_add_entry(NotificationRenderModel &service, const NotificationRecord &record) {
...
bool notification_content_matches(const NotificationEntry &entry, const NotificationRecord &record) {
    return entry.app_name == record.app_name && entry.summary == record.summary && entry.body == record.body;
}

} // namespace

void notification_sync(NotificationRenderModel &service, const NotificationService &notifications) {
    for (auto it = notifications.records.rbegin(); it != notifications.records.rend(); ++it) {
        const NotificationRecord &record = *it;
        auto e = std::find_if(service.entries.begin(), service.entries.end(), [&record](const NotificationEntry &en) { return en.id == record.id; });
        if (e == service.entries.end()) {
    ...
    for (NotificationEntry &entry : service.entries) {
        if (entry.exiting)
            continue;
        bool present = std::any_of(notifications.records.begin(), notifications.records.end(), [&entry](const NotificationRecord &r) { return r.id == entry.id; });
        if (!present)
...
void notification_paint(NotificationView &view, NotificationRenderModel &service) {
    ...
    for (NotificationEntry &entry : service.entries) {
        ...
        if (auto it = view.local_exit.find(entry.id); it != view.local_exit.end()) {
            if (it->second <= 0.0f)
                continue;
        ...
        Node *card = node_add_rrect(&view.scene.root, 0, card_y, kNotificationSurfaceWidth, entry.height, kNotificationCardRadius, kNotificationCardBorderWidth, rgba(card_fill), rgba(card_border));
        card->clip_children = true;

        float progress = std::clamp(entry.progress, 0.0f, 1.0f);
        float bar_rrect_h = 2.0f * kNotificationCardRadius;
        blend.push_back(with_alpha(urgency_color, kNotificationProgressTrackOpacity * card_opacity));
        Node *track_clip = node_add_group(card, 0, 0, kNotificationSurfaceWidth, kNotificationProgressHeight, true);
        node_add_rrect(track_clip, 0, 0, kNotificationSurfaceWidth, bar_rrect_h, kNotificationCardRadius, 0, rgba(blend.back()), kNodeTransparent);

        float fill_x = kNotificationSurfaceWidth * (1.0f - progress);
        float fill_w = kNotificationSurfaceWidth * progress;
        blend.push_back(with_alpha(urgency_color, card_opacity));
        Node *fill_clip = node_add_group(card, fill_x, 0, fill_w, kNotificationProgressHeight, true);
        node_add_rrect(fill_clip, -fill_x, 0, kNotificationSurfaceWidth, bar_rrect_h, kNotificationCardRadius, 0, rgba(blend.back()), kNodeTransparent);
        ...
        blend.push_back(with_alpha(urgency_color, card_opacity));
        node_add_rrect(card, content_x, content_y + (header_h - kNotificationUrgencyDotSize) / 2.0f, kNotificationUrgencyDotSize, kNotificationUrgencyDotSize, kNotificationUrgencyDotRadius, 0, rgba(blend.back()), kNodeTransparent);

        if (entry.app_name_texture.id) {
            blend.push_back(with_alpha(palette::text, 0.65f * card_opacity));
            node_add_texture(card, content_x + kNotificationUrgencyDotSize + kNotificationHeaderSpacing, content_y + (header_h - notification_detail_texture_height(entry.app_name_texture)) / 2.0f, entry.app_name_texture, rgba(blend.back()));
        }
        ...
        if (entry.summary_texture.id) {
            ...
            node_add_texture(card, content_x, row_y, entry.summary_texture, rgba(blend.back()));
            row_y += notification_detail_texture_height(entry.summary_texture);
        }
        if (entry.body_texture.id) {
            row_y += kNotificationContentSpacing;
            blend.push_back(with_alpha(palette::text, 0.72f * card_opacity));
            node_add_texture(card, content_x, row_y, entry.body_texture, rgba(blend.back()));
        }
        ...
        if (glyph.id) {
            float gw = static_cast<float>(glyph.width) / static_cast<float>(glyph.scale > 0 ? glyph.scale : 1);
            float gh = static_cast<float>(glyph.height) / static_cast<float>(glyph.scale > 0 ? glyph.scale : 1);
            float opacity = card_opacity * (entry.id == view.hovered_close_id ? 1.0f : kNotificationCloseIconOpacityIdle);
            blend.push_back(with_alpha(palette::text, opacity));
            node_add_texture(card, std::round(close_x + (kNotificationCloseHitAreaSize - gw) / 2.0f), std::round((kNotificationCloseHitAreaSize - gh) / 2.0f), glyph, rgba(blend.back()));
        }
