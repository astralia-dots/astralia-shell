...
bool launcher_init_egl(LauncherState &state, Renderer &renderer, EGLDisplay display, EGLConfig config, EGLContext context) {
    ...
    for (int i = 0; i < kLauncherMaxVisible; ++i)
        state.bullet_tex[i] = load_image_texture(ASTRALIA_SHELL_BULLET_DIR "/C" + std::to_string(i + 1) + ".png");
...
int launcher_row_at(const LauncherState &state, double px, double py) {
    for (const LauncherRowHit &h : state.row_hitboxes) {
        const Rect &r = h.rect;
...
void launcher_handle_pointer_move(LauncherState &state, wl_surface *focused_surface, double px, double py) {
    ...
    if (!state.open || focused_surface != state.surface)
        ...
        state.hovered_index = launcher_row_at(state, px, py);
...
void launcher_paint(LauncherState &state) {
    ...
    if (state.open) {
        ...
        float row_bg_w =
            box_x + kLauncherSurfaceWidth - kLauncherMenuPad - content_x;
        ...
        for (int i = 0; i < static_cast<int>(rows.size()); ++i) {
            ...
            if (!rows[i].subtitle.empty()) {
                ...
                float top =
                    y +
                    (kLauncherRowHeight - th - kLauncherTwoLineGap - sh) / 2.0f;
                if (label)
