...
const std::u32string &matrix_rain_glyph_pool() {
...
char32_t MatrixRain::random_glyph() const {
    const std::u32string &pool = matrix_rain_glyph_pool();
    if (pool.empty())
        return U' ';
    size_t idx =
...
void MatrixRain::rebuild(int width, int height, bool async_speed) {
    width_ = std::max(1, width);
    height_ = std::max(1, height);

    column_count_ =
        std::max(1, static_cast<int>(width_ / (kMatrixRainCellWidth * 2)));
    row_count_ = std::max(1, static_cast<int>(height_ / kMatrixRainCellHeight));
    ...
    float content_width =
        column_count_ * kMatrixRainCellWidth * 2 - kMatrixRainCellWidth;
    offset_x_ = (width_ - content_width) / 2.0f;
    offset_y_ = (height_ - row_count_ * kMatrixRainCellHeight) / 2.0f;
...
void MatrixRain::tick() {
    ...
    for (int c = 0; c < column_count_; ++c) {
        ...
        float x = offset_x_ + c * kMatrixRainCellWidth * 2.0f;
        ...
        if (head >= 0.0f) {
            float y = offset_y_ + head * kMatrixRainCellHeight;
            bool bold = random01() < kMatrixRainBoldChance;
            char32_t glyph = random_glyph();
            draw_glyph_centered(cr, glyph, x, y, bold, palette::text);
            col.last_glyph = glyph;
            col.last_head_drop = head;
