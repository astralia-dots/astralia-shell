...
std::string settings_detail_format_field(const Config &cfg, SettingsFieldId id, const std::string &monitor) {
    ...
    case SettingsFieldId::ScreensaverTimeout:
        return std::to_string(screensaver_effective_timeout_seconds(cfg, monitor));
...
bool settings_init_egl(SettingsState &state, const Config &cfg, Renderer &renderer, EGLDisplay display, EGLConfig config, EGLContext context, std::function<std::vector<std::string>()> monitor_names_fn, std::function<std::string()> focused_monitor_fn, std::function<MediaDecodeStatus(const std::string &, int)> wallpaper_decode_status_fn) {
    ...
    state.base.frame_clock.draw = [&state, &cfg, monitor_names_fn, focused_monitor_fn] {
...
void settings_commit_focused_field(SettingsState &state, const Config &cfg, const SettingsCommitFn &on_commit) {
    if (state.focused_field == SettingsFieldId::None)
        return;
    Config updated = cfg;
    settings_service_apply_field_text(updated, state.focused_field, state.field_buffer.text, state.idle_selected_monitor);
    on_commit(updated);
    state.focused_field = SettingsFieldId::None;
    state.field_buffer.preedit.clear();
    text_field_type_anim_clear(state.field_anim, state.base.animations, kSettingsFieldTypeAnimOwnerBase);
...
void settings_focus_field(SettingsState &state, const Config &cfg, const SettingsCommitFn &on_commit, SettingsFieldId id) {
    ...
    text_field_type_anim_settle(state.field_anim, state.base.animations, kSettingsFieldTypeAnimOwnerBase, state.field_buffer.text);
    if (state.sync_text_input_focus)
...
void settings_handle_click(SettingsState &state, const Config &cfg, const SettingsCommitFn &on_commit, double px, double py) {
    ...
    for (const PanelClickRegion &region : state.click_regions) {
        if (!hit(region.rect, px, py))
            continue;
        klog("settings: click (%.0f,%.0f) hit region kind=%d", px, py, static_cast<int>(region.kind));
        switch (region.kind) {
...
void settings_handle_key_event(SettingsState &state, const Config &cfg, const SettingsCommitFn &on_commit, const KeyEvent &event) {
    ...
    case TextFieldResult::Committed:
        klog("settings: field %d committed", static_cast<int>(state.focused_field));
        settings_commit_focused_field(state, cfg, on_commit);
        settings_request_frame(state);
        break;
    case TextFieldResult::Cancelled:
        klog("settings: field %d edit cancelled", static_cast<int>(state.focused_field));
        state.focused_field = SettingsFieldId::None;
        text_field_type_anim_clear(state.field_anim, state.base.animations, kSettingsFieldTypeAnimOwnerBase);
        settings_request_frame(state);
        break;
...
float draw_profile_block(SettingsState &state, Node *parent, int32_t scale, float x, float y, float w, bool expanded) {
    float avatar_x = x + (w - kSettingsProfileAvatarSize) / 2.0f;
    float avatar_y = y + kSettingsProfileTopPadding;
    animated_image_draw(state.profile_pic, parent, avatar_x, avatar_y, kSettingsProfileAvatarSize, kSettingsProfileAvatarSize, 1.0f);
    if (state.profile_pic.frames.empty()) {
        const Texture *avatar_icon =
            cached_icon(state.tcache, icon::user, scale);
        if (avatar_icon)
            node_add_texture(parent, avatar_x + (kSettingsProfileAvatarSize - avatar_icon->width) / 2.0f, avatar_y + (kSettingsProfileAvatarSize - avatar_icon->height) / 2.0f, *avatar_icon, rgba(palette::text));
    }
    ...
    if (expanded) {
        ...
        if (name_tex) {
            node_add_texture(parent, x + (w - name_tex->width) / 2.0f, info_y, *name_tex, rgba(palette::text));
            info_y += name_tex->height + kSettingsProfileLineGap;
        }
        if (uptime_tex) {
            node_add_texture(parent, x + (w - uptime_tex->width) / 2.0f, info_y, *uptime_tex, rgba(palette::text_dim));
...
void draw_nav_rail(SettingsState &state, Node *parent, int32_t scale, float x, float y, float w, float h, bool expanded) {
    node_add_rrect(parent, x, y, w, h, metrics::radius_md, 0.0f, rgba(palette::text_alpha04), kPanelNoBorder);
    ...
    for (int i = 0; i < kSettingsTabCount; ++i) {
        ...
        node_add_rrect(parent, x, row_y, w, kSettingsRailItemHeight, metrics::radius_sm, 0.0f, active ? rgba(palette::accent_alpha19) : kPanelNoBorder, kPanelNoBorder);

        float icon_x = x + kSettingsRailPadding;
        const Texture *icon_tex =
            cached_icon(state.tcache, kSettingsTabs[i].icon, scale);
        if (icon_tex)
            node_add_texture(parent, icon_x, row_y + (kSettingsRailItemHeight - icon_tex->height) / 2.0f, *icon_tex, row_color);
        ...
        if (expanded) {
            ...
            if (label_tex)
                node_add_texture(parent, label_x, row_y + (kSettingsRailItemHeight - label_tex->height) / 2.0f, *label_tex, row_color);
        }

        state.click_regions.push_back({PanelClickKind::TabSelect, {x, row_y, w, kSettingsRailItemHeight}, std::to_string(i)});
...
void draw_toggle_switch(SettingsState &state, Node *parent, float x, float y, bool active, const char *tag) {
    panel_draw_toggle_switch(parent, state.click_regions, x, y, kSettingsToggleTrackWidth, kSettingsToggleTrackHeight, kSettingsToggleKnobSize, kSettingsToggleKnobInset, active, PanelClickKind::ToggleFlip, tag);
}
...
void draw_toggle_row(SettingsState &state, Node *parent, int32_t scale, float x, float y, float w, const std::string &label, bool value, const char *tag, bool tiled) {
    ...
    if (tiled)
        node_add_rrect(parent, x, y, w, row_h, kSettingsTileRadius, kSettingsToggleTileBorderWidth, rgba(palette::text_alpha04), rgba(palette::text_alpha07));

    const Texture *label_tex = cached_text(state.tcache, label, scale);
    if (label_tex)
        node_add_texture(parent, x + inset, y + (row_h - label_tex->height) / 2.0f, *label_tex, tiled ? rgba(palette::text_alpha85) : rgba(palette::text));
...
void settings_paint(SettingsState &state, const Config &cfg, const std::vector<std::string> &monitor_names, const std::string &focused_monitor) {
    ...
    auto sync_region_selection = [&](WallpaperSubtabState &sub, bool animated) {
        ...
        } else if (sub.selected_region.empty() || std::find(monitor_names.begin(), monitor_names.end(), sub.selected_region) == monitor_names.end()) {
            sub.selected_region =
                !focused_monitor.empty() && std::find(monitor_names.begin(), monitor_names.end(), focused_monitor) != monitor_names.end()
                    ? focused_monitor
                : monitor_names.empty() ? ""
                                        : monitor_names[0];
        }
        int count = sub.selected_region.empty()
                        ? 1
                        : wallpaper_service_column_count(cfg, sub.selected_region, animated);
    ...
    if (!state.displays_selected_monitor.empty() && std::find(monitor_names.begin(), monitor_names.end(), state.displays_selected_monitor) == monitor_names.end())
        state.displays_selected_monitor.clear();
    if (!state.idle_selected_monitor.empty() && std::find(monitor_names.begin(), monitor_names.end(), state.idle_selected_monitor) == monitor_names.end())
    ...
    if (state.base.opacity > 0.0f) {
        float panel_w = std::min(static_cast<float>(state.base.width) - kSettingsWindowCardWidthInset, kSettingsWindowCardMaxWidth);
        float panel_h = std::min(static_cast<float>(state.base.height) - kSettingsWindowCardHeightInset, kSettingsWindowCardMaxHeight);
        ...
        panel_draw_box(root, panel_x, panel_y, panel_w, panel_h, metrics::border_thick);
        panel_draw_header(root, state.tcache, scale, "Settings", panel_x, panel_y, panel_w, state.click_regions);

        float content_y = panel_y + kPanelPadding + kPanelHeaderHeight + kPanelHeaderDividerGap;
        node_add_rect(root, panel_x + kPanelPadding, content_y, panel_w - kPanelPadding * 2.0f, 1.0f, rgba(palette::text_alpha11));
        content_y += 1.0f + kPanelContentGap;

        bool rail_expanded = panel_w > kSettingsNavRailCollapseBreakpoint;
        float rail_width = rail_expanded ? kSettingsNavRailExpandedWidth : kSettingsNavRailCollapsedWidth;

        float rail_x = panel_x + kPanelPadding;
        float profile_h = draw_profile_block(state, root, scale, rail_x, content_y, rail_width, rail_expanded);
        float rail_y = content_y + profile_h;
        float rail_h = panel_y + panel_h - kPanelPadding - rail_y;
        draw_nav_rail(state, root, scale, rail_x, rail_y, rail_width, rail_h, rail_expanded);

        float divider_x = rail_x + rail_width + kSettingsRailDividerGap;
        node_add_rect(root, divider_x, content_y, 1.0f, rail_h, rgba(palette::text_alpha11));
