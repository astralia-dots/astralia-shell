...
void idle_overlay_paint(IdleOverlayState &state) {
    ...
    if (state.screensaver_opacity > 0.0f) {
        ...
        node_add_rect(&state.scene.root, 0, 0, static_cast<float>(state.width), static_cast<float>(state.height), rgba(black));
...
void idle_overlay_set_active(IdleOverlayState &state, bool ambient_active, bool screensaver_active) {
    ...
    if (ambient_shown != state.ambient_active) {
        ...
        state.animations.animate(state.ambient_opacity, ambient_shown ? 1.0f : 0.0f, kIdleOverlayFadeMs, Easing::EaseOutCubic, [&state](float v) { state.ambient_opacity = v; }, {}, kIdleAmbientFadeOwner);
        changed = true;
    }
    if (screensaver_active != state.screensaver_active) {
        ...
        state.animations.animate(state.screensaver_opacity, screensaver_active ? 1.0f : 0.0f, kIdleOverlayFadeMs, Easing::EaseOutCubic, [&state](float v) { state.screensaver_opacity = v; }, {}, kIdleScreensaverFadeOwner);
        changed = true;
    }
    if (changed)
