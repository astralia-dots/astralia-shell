...
NotificationRecord *find_record(NotificationService &service, uint32_t id) {
    auto it =
        std::find_if(service.records.begin(), service.records.end(), [id](const NotificationRecord &r) { return r.id == id; });
    return it == service.records.end() ? nullptr : &*it;
}

void apply_record(NotificationRecord &record, const std::string &app_name, const std::string &summary, const std::string &body, uint8_t urgency, int32_t expire_timeout_ms) {
...
uint32_t notification_service_push(NotificationService &service, const std::string &app_name, const std::string &summary, const std::string &body, int32_t expire_timeout_ms, uint32_t id, uint8_t urgency) {
    if (id == 0)
        id = service.next_id++;
    ...
    if (NotificationRecord *existing = find_record(service, id)) {
        apply_record(*existing, app_name, summary, body, urgency, expire_timeout_ms);
    ...
    klog("notification: id=%u app='%s' summary='%s'", id, app_name.c_str(), summary.c_str());
...
bool notification_service_close(NotificationService &service, uint32_t id) {
    ...
    std::erase_if(service.records, [id](const NotificationRecord &r) { return r.id == id; });
...
bool notification_service_init(NotificationService &service, const std::function<void()> &on_change) {
    try {
        ...
        service.object = sdbus::createObject(*service.bus, sdbus::ObjectPath{"/org/freedesktop/Notifications"});
        ...
        service.bus->requestName(sdbus::ServiceName{"org.freedesktop.Notifications"});
