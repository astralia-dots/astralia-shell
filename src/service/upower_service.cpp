    ...
    try {
        state.device->callMethodAsync("GetAll")
            ...
            .uponReplyInvoke([&state](std::optional<sdbus::Error> err, std::map<std::string, sdbus::Variant> props) {
                if (err) {
                    klog("upower: property read failed (%s): %s", err->getName().c_str(), err->getMessage().c_str());
                    return;
                }
                if (!state.device)
                    return;
                ...
                if (auto it = props.find("State"); it != props.end())
                    if (auto v = variant_get<uint32_t>(it->second)) {
                        charging = *v == static_cast<uint32_t>(UpowerDeviceState::Charging);
                        full = *v == static_cast<uint32_t>(UpowerDeviceState::FullyCharged);
                    }
                if (auto it = props.find("Percentage"); it != props.end())
                    if (auto v = variant_get<double>(it->second))
                        percent = static_cast<int>(std::lround(*v));

                if (present != state.present || charging != state.charging || full != state.full || percent != state.percent) {
                    state.dirty = true;
                }
        ...
        klog("upower: GetAll dispatch failed (%s): %s", e.getName().c_str(), e.getMessage().c_str());
    }
}
...
void register_device(UpowerState &state, const std::string &path) {
    ...
    entry->path = path;
    try {
        entry->proxy = sdbus::createProxy(*state.bus, sdbus::ServiceName{kService}, sdbus::ObjectPath{path});
    } catch (const sdbus::Error &e) {
        klog("upower: failed to create proxy for %s: %s", path.c_str(), e.getMessage().c_str());
        return;
    }
    ...
    slot->proxy->uponSignal("PropertiesChanged")
        .onInterface(kPropertiesIface)
        .call([&state, slot](const std::string &, const std::map<std::string, sdbus::Variant> &, const std::vector<std::string> &) {
            refresh_device(state, *slot);
        });
}
...
bool upower_init(UpowerState &state) {
    try {
        ...
        state.manager =
            sdbus::createProxy(*state.bus, sdbus::ServiceName{kService}, sdbus::ObjectPath{kManagerPath});
        ...
        state.device = sdbus::createProxy(*state.bus, sdbus::ServiceName{kService}, device_path);

        state.device->uponSignal("PropertiesChanged")
            .onInterface(kPropertiesIface)
            .call([&state](const std::string &, const std::map<std::string, sdbus::Variant> &, const std::vector<std::string> &) { refresh(state); });
        ...
        state.manager->uponSignal("PropertiesChanged")
            .onInterface(kPropertiesIface)
            .call([&state](const std::string &, const std::map<std::string, sdbus::Variant> &, const std::vector<std::string> &) {
        ...
        refresh_on_battery(state);
        state.dirty = false;
        klog("upower: connected, display device at %s, %zu device(s) enumerated", device_path.c_str(), state.devices.size());
        return true;
    } catch (const sdbus::Error &e) {
        klog("upower: connection failed (%s): %s - no battery info available", e.getName().c_str(), e.getMessage().c_str());
        state.device.reset();
        state.manager.reset();
        state.bus.reset();
        return false;
